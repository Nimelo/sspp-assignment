  ...
  namespace common {
    ...
    public:
      ...
      static CRS<VALUE_TYPE> transform(MatrixMarket<VALUE_TYPE> & matrix_market) {
        std::vector<MatrixMarketTuple<VALUE_TYPE>> tuples = matrix_market.GetTuples();
        std::sort(tuples.begin(), tuples.end(), MatrixMarket<VALUE_TYPE>::CompareMatrixMarketTuple);
        ...
        while(row_start_indexes_index < matrix_market.GetRows() + 1
          ...
          values[non_zeros_index - 1] = tuples[non_zeros_index - 1].GetValue();
